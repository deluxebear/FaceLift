                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    if vm.selectedTab == .passcodeThemes {
                        if vm.passcodeTabMode == .themeCreator {
                            ...
                            if count > 0 {
                                Text(L("Theme Creator · %@ of 10 keys configured · Target: %@", "\(count)", targetInfo))
                                    .font(.system(size: 10))
                                    .foregroundColor(.secondary)
                            } else {
                                Text(L("Theme Creator · Import a poster or drop icons onto keys"))
                                    .font(.system(size: 10))
                                    .foregroundColor(.secondary)
                            }
                        } else if let theme = vm.loadedPasscodeTheme {
                            let targetInfo = "\(vm.targetTelephonyVersion) · \(vm.passcodeLanguageTarget.code.uppercased()) · \(vm.passcodeBoldTarget.code)"
                            Text(L("%@ source assets loaded · Target: %@", "\(theme.fileCount)", targetInfo))
                                .font(.system(size: 10))
                                .foregroundColor(.secondary)
                        } else {
                            Text(L("No .passthm loaded · Select a theme package to flash"))
                                .font(.system(size: 10))
                                .foregroundColor(.secondary)
                        }
                    } else if !vm.cards.isEmpty {
                        Text(L("%@ of %@ cards selected · %@ ready to flash", "\(vm.cards.filter { $0.isSelected }.count)", "\(vm.cards.count)", "\(readyToFlashCount)"))
                            .font(.system(size: 10))
                            .foregroundColor(.secondary)
                    }
                }
                ...
                if vm.selectedTab == .passcodeThemes {
                    ...
                    Button(action: { vm.applySkin() }) {
                        HStack(spacing: 6) {
                            ...
                            Text(vm.isFlashing ? L("Flashing Cards...") : (readyToFlashCount > 0 ? L("Flash Skins (%@ Cards)", "\(readyToFlashCount)") : L("Flash Skins")))
                                .fontWeight(.semibold)
                        }
                        .padding(.horizontal, 8)
                    }

...
static int StreamDeviceLogs(AMDServiceConnectionRef connection) {
    ...
    if (AMDServiceConnectionSendMessage(connection,
        ...
        fprintf(stderr, "FaceLift scanner: Could not request device log streaming.\n");
        return 2;
    }
    ...
    if (![status isKindOfClass:NSDictionary.class] ||
        ...
        fprintf(stderr, "FaceLift scanner: %s\n",
                (error ?: @"The device refused to start log streaming.").UTF8String);
        return 2;
    }

    fprintf(stderr, "FaceLift scanner: Connected to the unified device log stream.\n");
    while (YES) {
        @autoreleasepool {
...
static int RunSyslog(void) {
    if (FindTarget() != 0 || !TargetDevice) {
        fprintf(stderr, "FaceLift scanner: iPhone not found. Reconnect it via USB.\n");
        return 2;
    }
    AMDeviceRef device = TargetDevice;
    if (AMDeviceConnect(device) != 0) {
        fprintf(stderr, "FaceLift scanner: Could not connect to the iPhone.\n");
        return 2;
    }
    if (!AMDeviceIsPaired(device)) AMDevicePair(device);
    if (AMDeviceValidatePairing(device) != 0 || AMDeviceStartSession(device) != 0) {
        fprintf(stderr, "FaceLift scanner: Unlock the iPhone and trust this Mac, then retry.\n");
    ...
    if (AMDeviceSecureStartService(
        ...
        !connection) {
        fprintf(stderr, "FaceLift scanner: Could not open the device log service. Unlock the iPhone and retry.\n");
